...
est clear
estpost tabstat emp_status monthly_earnings wage remote_any remote_primary remote_some days_remote days_worked remote_fraction weekly_hours age female hispanic black asian hsdropout hsgraduate somecollege associates bachelors highered [aweight=person_weight], c(stat) stat(n mean sd min max)
esttab using "$tables\sum_stats_keep_ue.tex", replace   ///
 ...
 title("Summary Stats - Keeping Unemployed")
...
est clear
estpost tabstat emp_status monthly_earnings wage days_remote days_worked remote_fraction weekly_hours age female hispanic black asian hsdropout hsgraduate somecollege associates bachelors highered [aweight=person_weight], by(remote_status) c(stat) stat(mean) nototal
esttab using "$tables\sum_stats_by_remote_keep_ue.tex", replace   ///
 ...
 postfoot(`"\end{tabular}"') ///
 nomtitles ///
 title("Summary Stats by Remote Status - Keeping Unemployed")
...
graph twoway connected remote_any remote_some remote_primary date, title("Work-from-Home by Year - Keeping Unemployed")legend(order(1 "Any WFH" 2 "Hybrid" 3 "Primarily WFH")) graphregion(fcolor(white))
graph close
restore

* Graph sample unemployment rate
preserve
collapse (mean) emp_status [aweight = person_weight], by(date)
gen unemp_rate = 1-emp_status
graph twoway connected unemp_rate date, title("Sample Unemployment Rate") graphregion(fcolor(white))
graph close
restore

* Graph sample unemployment rate by remote status by year
preserve
gen unemp_rate_remote_none = 1-emp_status if remote_any==0
gen unemp_rate_remote_any = 1-emp_status if remote_any==1
gen unemp_rate_remote_some = 1-emp_status if remote_some==1
gen unemp_rate_remote_primary = 1-emp_status if remote_primary==1
collapse (mean) unemp_rate_remote_none unemp_rate_remote_any unemp_rate_remote_some unemp_rate_remote_primary [aweight=person_weight], by(date)
graph twoway connected unemp_rate_remote_none unemp_rate_remote_some unemp_rate_remote_primary date, title("Sample Unemployment Rate - By Remote Status") legend(order(1 "In-Person" 2 "Hybrid" 3 "Primarily WFH")) graphregion(fcolor(white))
graph close
restore
...
graph twoway connected remote_fraction date, ytitle("") title("% of time Working-from-Home - Keeping Unemployed") graphregion(fcolor(white))
graph close
restore
...
graph twoway connected monthly_earnings_remote_none monthly_earnings_remote_some monthly_earnings_remote_primary date, title("Median Monthly Earnings - Keeping Unemployed") legend(order(1 "In-Person" 2 "Hybrid" 3 "Primarily WFH")) graphregion(fcolor(white))
graph close
restore
...
graph twoway connected wage_remote_none wage_remote_some wage_remote_primary date, title("Median Hourly Wage - Keeping Unemployed") legend(order(1 "In-Person" 2 "Hybrid" 3 "Primarily WFH")) graphregion(fcolor(white))
graph close
restore
...
graph twoway connected weekly_hours_remote_none weekly_hours_remote_some weekly_hours_remote_primary date, title("Avg. Weekly Hours - Keeping Unemployed") legend(order(1 "In-Person" 2 "Hybrid" 3 "Primarily WFH")) graphregion(fcolor(white))
graph close
restore
...
graph twoway connected college_degree_remote_none college_degree_remote_some college_degree_remote_primary date, title("Bachelor's Degree or Higher - Keeping Unemployed") legend(order(1 "In-Person" 2 "Hybrid" 3 "Primarily WFH")) graphregion(fcolor(white))
graph close
restore
...
graph twoway connected female_remote_none female_remote_some female_remote_primary date, title("% Female - Keeping Unemployed") legend(order(1 "In-Person" 2 "Hybrid" 3 "Primarily WFH")) graphregion(fcolor(white))
graph close
restore
...
esttab using "$tables\sum_stats_ann_keep_ue.tex", replace   ///
 cells("count(fmt(%9.3gc)) mean(fmt(%9.2fc)) sd(fmt(%9.2fc)) min max") nonumber /// 
 nomtitle nonote noobs label booktabs ///
 collabels("N" "Mean" "SD" "Min" "Max") ///
 prehead(`"\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}"' ///
         `"\begin{tabular}{l*{1}{ccccc}}"') ///
 postfoot(`"\end{tabular}"') ///
 title("Summary Stats (Annual) - Keeping Unemployed")
...
esttab using "$tables\sum_stats_by_remote_ann_keep_ue.tex", replace   ///
 ...
 postfoot(`"\end{tabular}"') ///
 nomtitles ///
 title("Summary Stats by Remote Status (Annual) - Keeping Unemployed")
...
graph twoway connected remote_any remote_some remote_primary year, title("Work-from-Home by Year - Keeping Unemployed")legend(order(1 "Any WFH" 2 "Hybrid" 3 "Primarily WFH")) graphregion(fcolor(white))
graph close
restore
...
graph twoway connected remote_fraction year, ytitle("") title("% of time Working-from-Home - Keeping Unemployed") graphregion(fcolor(white))
graph close
restore
...
graph twoway connected annual_earnings_remote_none annual_earnings_remote_some annual_earnings_remote_primary year, title("Median Annual Earnings - Keeping Unemployed") legend(order(1 "In-Person" 2 "Hybrid" 3 "Primarily WFH")) graphregion(fcolor(white))
graph close
restore
...
graph twoway connected wage_remote_none wage_remote_some wage_remote_primary year, title("Median Hourly Wage - Keeping Unemployed") legend(order(1 "In-Person" 2 "Hybrid" 3 "Primarily WFH")) graphregion(fcolor(white))
graph close
restore
...
graph twoway connected weekly_hours_remote_none weekly_hours_remote_some weekly_hours_remote_primary year, title("Avg. Weekly Hours - Keeping Unemployed") legend(order(1 "In-Person" 2 "Hybrid" 3 "Primarily WFH")) graphregion(fcolor(white))
graph close
restore
...
graph twoway connected college_degree_remote_none college_degree_remote_some college_degree_remote_primary year, title("Bachelor's Degree or Higher - Keeping Unemployed") legend(order(1 "In-Person" 2 "Hybrid" 3 "Primarily WFH")) graphregion(fcolor(white))
graph close
restore
...
graph twoway connected female_remote_none female_remote_some female_remote_primary year, title("% Female - Keeping Unemployed") legend(order(1 "In-Person" 2 "Hybrid" 3 "Primarily WFH")) graphregion(fcolor(white))
graph close
restore