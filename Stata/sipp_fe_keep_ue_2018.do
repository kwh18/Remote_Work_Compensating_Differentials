...
esttab using "$tables\fe_keep_ue_all_2018.tex", replace f ///
...
eststo: reghdfe emp_status remote_any job_change_year remote_any_job_change_year age age_sq i.household_relationship i.worker_class i.occupation_code_agg i.industry_code_agg i.state_code female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered DN_class cov_rate death_rate [aweight=person_weight], absorb(person_id date) vce(robust)
 ...
 esttab using "$tables\fe_keep_ue_all_2018.tex", append f ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
...
esttab using "$tables\fe_keep_ue_job_change_2018.tex", replace f ///
 prehead(\begin{tabular}{l*{@M}{r}} \toprule) ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
...
* Employment
* Fixed Effects
eststo: reghdfe emp_status remote_any remote_any_2019 age age_sq i.household_relationship i.worker_class i.occupation_code_agg i.industry_code_agg i.state_code female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered DN_class cov_rate death_rate [aweight=person_weight], absorb(person_id date) vce(robust)
...
esttab using "$tables\fe_keep_ue_year_inter_2018.tex", replace f ///
 prehead(\begin{tabular}{l*{@M}{r}} \toprule) ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
...
esttab using "$tables\fe_keep_ue_job_change_year_inter_2018.tex", replace f ///
 prehead(\begin{tabular}{l*{@M}{r}} \toprule) ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
