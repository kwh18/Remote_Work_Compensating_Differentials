...
replace tpearn_alt=0 if tpearn_alt==. & emp_status==0
replace annual_hours=0 if annual_hours==. & emp_status==0
replace tjb1_jobhrs1=0 if tjb1_jobhrs1==. & emp_status==0
...
egen min_ln_annual_earnings = min(ln_annual_earnings)
replace ln_annual_earnings = min_ln_annual_earnings if annual_earnings==0
gen ln_wage = ln(wage)
...
save sipp_clean_annual_keep_ue, replace