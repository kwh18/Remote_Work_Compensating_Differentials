...
esttab using "$tables\main_keep_ue_ivreghdfe_2020.tex", replace f ///
...
* Employment Status
* IV
eststo: ivreghdfe emp_status age age_sq i.household_relationship i.worker_class i.occupation_code_agg i.industry_code_agg i.state_code female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered (remote_any = stringency_index), absorb(person_id date) cluster(clust_iv)

* IV - Interaction instrument
eststo: ivreghdfe emp_status age age_sq i.household_relationship i.worker_class i.occupation_code_agg i.industry_code_agg i.state_code female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered (remote_any = iv_interact), absorb(person_id date) cluster(clust_iv2)

esttab using "$tables\main_keep_ue_ivreghdfe_hours_2020.tex", replace f ///
 prehead(\begin{tabular}{l*{@M}{r}} \toprule) ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
 keep(remote_any) varlabels(remote_any "Remote Work (Any)") ///
 label booktabs nonotes nonumbers noobs collabels(none) ///
 alignment(D{.}{.}{-1}) ///
 mtitles("N" "n" "IV" "IV2" "IV" "IV2") ///
 mgroups("" "Weekly Hours" "Employment Status", pattern(1 0 1 0 1 0) ///
 prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span}))
...
esttab using "$tables\main_keep_ue_ivreghdfe_2020.tex", append f ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
...
* Employment Status
* IV
eststo: ivreghdfe emp_status age age_sq i.household_relationship i.worker_class i.occupation_code_agg i.industry_code_agg i.state_code female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered (remote_some = stringency_index), absorb(person_id date) cluster(clust_iv)

* IV - Interaction instrument
eststo: ivreghdfe emp_status age age_sq i.household_relationship i.worker_class i.occupation_code_agg i.industry_code_agg i.state_code female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered (remote_some = iv_interact), absorb(person_id date) cluster(clust_iv2)

esttab using "$tables\main_keep_ue_ivreghdfe_hours_2020.tex", append f ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
 keep(remote_some) varlabels(remote_some "Remote Work (Some)") ///
 label booktabs nodep nonum nomtitles nolines nonotes noobs collabels(none) ///
 alignment(D{.}{.}{-1})
...
* Employment Status
* IV
eststo: ivreghdfe emp_status age age_sq i.household_relationship i.worker_class i.occupation_code_agg i.industry_code_agg i.state_code female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered (remote_primary = stringency_index), absorb(person_id date) cluster(clust_iv)

* IV - Interaction instrument
eststo: ivreghdfe emp_status age age_sq i.household_relationship i.worker_class i.occupation_code_agg i.industry_code_agg i.state_code female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered (remote_primary = iv_interact), absorb(person_id date) cluster(clust_iv2)
