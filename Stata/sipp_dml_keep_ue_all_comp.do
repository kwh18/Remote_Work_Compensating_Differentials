...
esttab using "$tables\DML_keep_ue_all_comp.tex", replace f ///
...
* Employment Status
* OLS
eststo: reg emp_status remote_any age age_sq i.household_relationship i.worker_class i.occupation_code_agg i.industry_code_agg i.state_code  female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered DN_class `components' [aweight=person_weight], r
...
* Double Machine Learning
eststo: xporegress emp_status remote_any, controls(age age_sq i.household_relationship i.worker_class i.occupation_code_agg i.industry_code_agg i.state_code i.date female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered DN_class `components')

esttab using "$tables\DML_keep_ue_all_comp_hours.tex", replace f ///
 prehead(\begin{tabular}{l*{@M}{r}} \toprule) ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
 keep(remote_any) varlabels(remote_any "Remote Work (Any)") ///
 label booktabs nonotes nonumbers noobs collabels(none) ///
...
esttab using "$tables\DML_keep_ue_all_comp.tex", append f ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
...
* Double Machine Learning
eststo: xporegress emp_status remote_some, controls(age age_sq i.household_relationship i.worker_class i.occupation_code_agg i.industry_code_agg i.state_code i.date female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered DN_class `components')

esttab using "$tables\DML_keep_ue_all_comp_hours.tex", append f ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
 keep(remote_some) varlabels(remote_some "Remote Work (Some)") ///
 label booktabs nodep nonum nomtitles nolines nonotes noobs collabels(none) ///
 alignment(D{.}{.}{-1})
...
* Double Machine Learning
eststo: xporegress emp_status remote_primary, controls(age age_sq i.household_relationship i.worker_class i.occupation_code_agg i.industry_code_agg i.state_code i.date female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered DN_class `components')
