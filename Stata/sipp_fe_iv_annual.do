...
esttab using "$tables\first_stage_iv_ann.tex", replace   ///
...
esttab using "$tables\first_stage_iv2_ann.tex", replace   ///
 b(3) se(3) star(* 0.10 ** 0.05 *** 0.01) /// 
...
esttab using "$tables\ann_2018.tex", replace f ///
...
esttab using "$tables\ann_2018_hours.tex", replace f ///
 prehead(\begin{tabular}{l*{@M}{r}} \toprule) ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
 keep(remote_any) varlabels(remote_any "Remote Work (Any)") ///
 label booktabs noobs nonotes nonumbers collabels(none) ///
...
esttab using "$tables\ann_2018.tex", append f ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
...
esttab using "$tables\ann_2018_hours.tex", append f ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
 keep(remote_some) varlabels(remote_some "Remote Work (Some)") ///
 label booktabs nodep nonum nomtitles nolines noobs nonotes collabels(none) ///
 alignment(D{.}{.}{-1})
...
esttab using "$tables\ACS_ann_2018.tex", replace f ///
 prehead(\begin{tabular}{l*{@M}{r}} \toprule) ///
 b(%9.3gc) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
