...
graph export "$tables\event_study_wages_remote_any_ann_keep_ue.png", replace as(png)
...
graph export "$tables\event_study_earnings_remote_any_ann_keep_ue.png", replace as(png)
...
graph export "$tables\event_study_hours_remote_any_ann_keep_ue.png", replace as(png)

* Employment Status
eventdd emp_status age age_sq i.state_code weekly_hours female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered i.household_relationship i.worker_class i.industry_code_agg i.occupation_code_agg i.year, timevar(dist_from_eyear_any) accum leads(2) lags(2) cluster(person_id) graph_op(legend(off) title( "{stSerif:{it:Trends in Employment Status at WFH Switch (Any)}}", color(black) size(large)) xtitle("{stSerif:Months Since WFH Switch}") xscale(titlegap(2)) note("{stSerif:{it:Notes}. Naive Event Study technique used. Controls include age, age^2, state of residence, weekly hours,}" "{stSerif:gender, race, education, household relationship, worker class, industry and occupation. year and individual}" "{stSerif:fixed effects included. Errors are clustered at the individual level.}", margin(small)) graphregion(fcolor(white) lcolor(white) lwidth(vvvthin) ifcolor(white) ilcolor(white) ilwidth(vvvthin)) mcolor(black))
graph export "$tables\event_study_emp_status_remote_any_ann_keep_ue.png", replace as(png)
...
graph export "$tables\event_study_wages_remote_some_ann_keep_ue.png", replace as(png)
...
graph export "$tables\event_study_earnings_remote_some_ann_keep_ue.png", replace as(png)
...
graph export "$tables\event_study_hours_remote_some_ann_keep_ue.png", replace as(png)

* Employment Status
eventdd emp_status age age_sq i.state_code weekly_hours female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered i.household_relationship i.worker_class i.industry_code_agg i.occupation_code_agg i.year, timevar(dist_from_eyear_some) accum leads(2) lags(2) cluster(person_id) graph_op(legend(off) title( "{stSerif:{it:Trends in Employment Status at WFH Switch (Some)}}", color(black) size(large)) xtitle("{stSerif:Months Since WFH Switch}") xscale(titlegap(2)) note("{stSerif:{it:Notes}. Naive Event Study technique used. Controls include age, age^2, state of residence, weekly hours,}" "{stSerif:gender, race, education, household relationship, worker class, industry and occupation. year and individual}" "{stSerif:fixed effects included. Errors are clustered at the individual level.}", margin(small)) graphregion(fcolor(white) lcolor(white) lwidth(vvvthin) ifcolor(white) ilcolor(white) ilwidth(vvvthin)) mcolor(black))
graph export "$tables\event_study_emp_status_remote_some_ann_keep_ue.png", replace as(png)
...
graph export "$tables\event_study_wages_remote_primary_ann_keep_ue.png", replace as(png)
...
graph export "$tables\event_study_earnings_remote_primary_ann_keep_ue.png", replace as(png)
...
graph export "$tables\event_study_hours_remote_primary_ann_keep_ue.png", replace as(png)

* Employment Status
eventdd emp_status age age_sq i.state_code weekly_hours female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered i.household_relationship i.worker_class i.industry_code_agg i.occupation_code_agg i.year, timevar(dist_from_eyear_primary) accum leads(2) lags(2) cluster(person_id) graph_op(legend(off) title( "{stSerif:{it:Trends in Employment Status at WFH Switch (Primary)}}", color(black) size(large)) xtitle("{stSerif:Months Since WFH Switch}") xscale(titlegap(2)) note("{stSerif:{it:Notes}. Naive Event Study technique used. Controls include age, age^2, state of residence, weekly hours,}" "{stSerif:gender, race, education, household relationship, worker class, industry and occupation. year and individual}" "{stSerif:fixed effects included. Errors are clustered at the individual level.}", margin(small)) graphregion(fcolor(white) lcolor(white) lwidth(vvvthin) ifcolor(white) ilcolor(white) ilwidth(vvvthin)) mcolor(black))
graph export "$tables\event_study_emp_status_remote_primary_ann_keep_ue.png", replace as(png)
...
graph export "$tables\event_study_wages_remote_any_ann_keep_ue_SA.png", replace as(png)
...
graph export "$tables\event_study_earnings_remote_any_ann_keep_ue_SA.png", replace as(png)
...
graph export "$tables\event_study_hours_remote_any_ann_keep_ue_SA.png", replace as(png)

* Employment Status
eventstudyinteract emp_status g_2_any g0_any g1_any g2_any g_1_any, cohort(eyear_any) control_cohort(control_any) covariates(age age_sq i.state_code weekly_hours female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered i.household_relationship i.worker_class i.industry_code_agg i.occupation_code_agg) absorb(i.year i.person_id) vce(cluster person_id)
...
coefplot matrix(B[1]), vertical se(B[2]) ciopts(recast(rcap)) legend(off) title( "{stSerif:{it:Trends in Employment at WFH Switch (Any)}}", color(black) size(large)) xtitle("{stSerif:Years Since WFH Switch}") note("{stSerif:{it:Notes}. Sun and Abraham (2020) technique used. Controls include age, age^2, state of residence, weekly hours,}" "{stSerif:gender, race, education, household relationship, worker class, industry and occupation. Year and individual}" "{stSerif:fixed effects included. Errors are clustered at the individual level.}", margin(small)) xscale(titlegap(2)) graphregion(fcolor(white) lcolor(white) lwidth(vvvthin) ifcolor(white) ilcolor(white) ilwidth(vvvthin)) mcolor(black) xline(2, lcolor(black) lwidth(thin)) yline(0)
graph export "$tables\event_study_emp_status_remote_any_ann_keep_ue_SA.png", replace as(png)
...
graph export "$tables\event_study_wages_remote_some_ann_keep_ue_SA.png", replace as(png)
...
graph export "$tables\event_study_earnings_remote_some_ann_keep_ue_SA.png", replace as(png)
...
graph export "$tables\event_study_hours_remote_some_ann_keep_ue_SA.png", replace as(png)

* Employment Status
eventstudyinteract emp_status g_2_some g0_some g1_some g2_some g_1_some, cohort(eyear_some) control_cohort(control_some) covariates(age age_sq i.state_code weekly_hours female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered i.household_relationship i.worker_class i.industry_code_agg i.occupation_code_agg) absorb(i.year i.person_id) vce(cluster person_id)
...
coefplot matrix(B[1]), vertical se(B[2]) ciopts(recast(rcap)) legend(off) title( "{stSerif:{it:Trends in Employment at WFH Switch (Some)}}", color(black) size(large)) xtitle("{stSerif:Years Since WFH Switch}") note("{stSerif:{it:Notes}. Sun and Abraham (2020) technique used. Controls include age, age^2, state of residence, weekly hours,}" "{stSerif:gender, race, education, household relationship, worker class, industry and occupation. Year and individual}" "{stSerif:fixed effects included. Errors are clustered at the individual level.}", margin(small)) xscale(titlegap(2)) graphregion(fcolor(white) lcolor(white) lwidth(vvvthin) ifcolor(white) ilcolor(white) ilwidth(vvvthin)) mcolor(black) xline(2, lcolor(black) lwidth(thin)) yline(0)
graph export "$tables\event_study_emp_status_remote_some_ann_keep_ue_SA.png", replace as(png)
...
graph export "$tables\event_study_wages_remote_primary_ann_keep_ue_SA.png", replace as(png)
...
graph export "$tables\event_study_earnings_remote_primary_ann_keep_ue_SA.png", replace as(png)
...
graph export "$tables\event_study_hours_remote_primary_ann_keep_ue_SA.png", replace as(png)

* Employment Status
eventstudyinteract emp_status g_2_primary g0_primary g1_primary g2_primary g_1_primary, cohort(eyear_primary) control_cohort(control_primary) covariates(age age_sq i.state_code weekly_hours female hispanic black asian resid_race hsgraduate somecollege associates bachelors highered i.household_relationship i.worker_class i.industry_code_agg i.occupation_code_agg) absorb(i.year i.person_id) vce(cluster person_id)
...
coefplot matrix(B[1]), vertical se(B[2]) ciopts(recast(rcap)) legend(off) title( "{stSerif:{it:Trends in Employment at WFH Switch (Any)}}", color(black) size(large)) xtitle("{stSerif:Years Since WFH Switch}") note("{stSerif:{it:Notes}. Sun and Abraham (2020) technique used. Controls include age, age^2, state of residence, weekly hours,}" "{stSerif:gender, race, education, household relationship, worker class, industry and occupation. Year and individual}" "{stSerif:fixed effects included. Errors are clustered at the individual level.}", margin(small)) xscale(titlegap(2)) graphregion(fcolor(white) lcolor(white) lwidth(vvvthin) ifcolor(white) ilcolor(white) ilwidth(vvvthin)) mcolor(black) xline(2, lcolor(black) lwidth(thin)) yline(0)
graph export "$tables\event_study_emp_status_remote_primary_ann_keep_ue_SA.png", replace as(png)
