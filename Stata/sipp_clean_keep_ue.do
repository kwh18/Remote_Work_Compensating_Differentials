...
gen monthly_hours = tjb1_wkhrs1 + tjb1_wkhrs2 + tjb1_wkhrs3 + tjb1_wkhrs4
replace monthly_hours = monthly_hours + tjb1_wkhrs5 if tjb1_wkhrs5!=.
...
* While we want unemployed obs, those unemployed for whole sample are not interesting comparison group
sort person_id emp_status
replace emp_status = emp_status - 1
tab emp_status
by person_id: egen max_emp_status = max(emp_status)
drop if missing(max_emp_status)
drop if max_emp_status==0
...
* Keeping series we care about
sum
keep state_code spanel erelrpe esex eorigin erace eeduc wpfinwgt ejb1_jobid ejb1_clwrk ejb1_dyswkd ejb1_dyswkdh ejb1_empsize rtanf_mnyn tjb1_occ tjb1_ind tjb1_jobhrs1 tjb2_jobhrs1 tage remote_any remote_some remote_primary monthcode tptotinc tpearn_alt monthly_hours ssuid pnum survey_year cal_year person_id modate emp_status
sum
...
// by person_id: drop if missing(erelrpe)
// by person_id: gen miss2 = !missing(erelrpe)
// by person_id: egen missing_erelrpe = min(miss2)
// drop if missing_erelrpe == 0
...
rename tage age
rename tptotinc monthly_earnings_total
rename tpearn_alt monthly_earnings
rename ejb1_empsize employer_size
rename ejb1_jobid jobid_1

* Fixing earnings and hours for unemployed
replace monthly_earnings=0 if monthly_earnings==. & emp_status==0
replace monthly_hours=0 if monthly_hours==. & emp_status==0
replace weekly_hours=0 if weekly_hours==. & emp_status==0

* Rough hourly wage calculation
gen wage = monthly_earnings/monthly_hours
replace wage = 0 if monthly_earnings==0
sum wage, d

* log on Income
gen ln_monthly_earnings = ln(monthly_earnings)
egen min_ln_monthly_earnings = min(ln_monthly_earnings)
replace ln_monthly_earnings = min_ln_monthly_earnings if monthly_earnings==0
gen ln_wage = ln(wage)
egen min_ln_wage = min(ln_wage)
replace ln_wage = min_ln_wage if wage==0
gen ln_weekly_hours = ln(weekly_hours)
egen min_ln_weekly_hours = min(ln_weekly_hours)
replace ln_weekly_hours = min_ln_weekly_hours if weekly_hours==0

* Generate variables as future earnings growth
gen ln_monthly_earnings_yoy_forward = F12.ln_monthly_earnings - ln_monthly_earnings
gen ln_wage_yoy_forward = F12.ln_wage - ln_wage
...
* Create Experience proxy using age and education level
gen experience_proxy = 0
replace experience_proxy = age - 16 if hsdropout==1
replace experience_proxy = age - 18 if hsgraduate==1
replace experience_proxy = age - 20 if somecollege==1 | associates==1
replace experience_proxy = age - 22 if bachelors==1
replace experience_proxy = age - 25 if highered==1
...
* Set industry and occupation of unemployed equal to last nonmissing value
by person_id (date), sort: replace occupation = occupation[_n-1] if occupation == ""
by person_id (date), sort: replace occupation_code = occupation_code[_n-1] if occupation_code >= .
by person_id (date), sort: replace occupation_code_agg = occupation_code_agg[_n-1] if occupation_code_agg >= .
by person_id (date), sort: replace industry = industry[_n-1] if industry == ""
by person_id (date), sort: replace industry_code = industry_code[_n-1] if industry_code >= .
by person_id (date), sort: replace industry_code_agg = industry_code_agg[_n-1] if industry_code_agg >= .

* Merging in 4-digit Dingel-Nieman Indicators
merge m:1 occupation_code using dingel_nieman_4_digit
drop if _merge==2
drop _merge
...
sort person_id date
gen remote_any_date = remote_any * date
replace remote_any_date =. if remote_any_date==0
by person_id: egen remote_any_date_min = min(remote_any_date)
gen remote_any_later = (date<remote_any_date_min) & !missing(remote_any_date_min)
gen remote_any_plus_later = remote_any_later + remote_any
drop remote_any_date remote_any_date_min

gen remote_some_date = remote_some * date
replace remote_some_date =. if remote_some_date==0
by person_id: egen remote_some_date_min = min(remote_some_date)
gen remote_some_later = (date<remote_some_date_min) & !missing(remote_some_date_min)
gen remote_some_plus_later = remote_some_later + remote_some
drop remote_some_date remote_some_date_min

gen remote_primary_date = remote_primary * date
replace remote_primary_date =. if remote_primary_date==0
by person_id: egen remote_primary_date_min = min(remote_primary_date)
gen remote_primary_later = (date<remote_primary_date_min) & !missing(remote_primary_date_min)
gen remote_primary_plus_later = remote_primary_later + remote_primary
drop remote_primary_date remote_primary_date_min

* Reordering Variables
order person_id date state_code cal_year person_weight monthly_earnings wage remote_any remote_some remote_primary days_remote days_worked age
sort person_id date
sum

save sipp_clean_keep_ue, replace
...
gen ind_change_show = d.industry_code_agg
gen occ_change_show = d.occupation_code_agg
gen ind_occ_change = (ind_change_show!=0 & ind_change_show!=.) | (occ_change_show!=0 & occ_change_show!=.)

* Make time series of industry, occupation, and industry x occupation for leaving probabilities
* Industry
sum job_change ind_occ_change
