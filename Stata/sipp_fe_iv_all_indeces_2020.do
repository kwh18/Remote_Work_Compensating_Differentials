...
esttab using "$tables\first_stage_iv_all_ind_2020.tex", replace   ///
...
esttab using "$tables\first_stage_iv2_all_ind_2020.tex", replace   ///
 b(3) se(3) star(* 0.10 ** 0.05 *** 0.01) /// 
...
esttab using "$tables\all_ind_2020.tex", replace f ///
...
esttab using "$tables\all_ind_2020_hours.tex", replace f ///
 prehead(\begin{tabular}{l*{@M}{r}} \toprule) ///
 b(3) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
 keep(remote_any) varlabels(remote_any "Remote Work (Any)") ///
 label booktabs noobs nonotes nonumbers collabels(none) ///
...
esttab using "$tables\all_ind_2020.tex", append f ///
 b(3) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
...
esttab using "$tables\all_ind_2020_hours.tex", append f ///
 b(3) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
 keep(remote_some) varlabels(remote_some "Remote Work (Some)") ///
 label booktabs nodep nonum nomtitles nolines noobs nonotes collabels(none) ///
 alignment(D{.}{.}{-1})
...
esttab using "$tables\ACS_all_ind_2020.tex", replace f ///
 prehead(\begin{tabular}{l*{@M}{r}} \toprule) ///
 b(3) se(3) star(* 0.10 ** 0.05 *** 0.01)  /// 
